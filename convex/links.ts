...
export const createLink = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await checkRateLimit(ctx, userId ?? args.ipHash ?? "anonymous", !!identity);
...
export const getUserLinks = query({
  args: {
    status: v.optional(
      v.union(v.literal("active"), v.literal("expired"), v.literal("all")),
    ),
  ...
  handler: async (ctx, args) => {
    ...
    if (args.dateFrom)
      links = links.filter((l: any) => l.createdAt >= args.dateFrom!);
    if (args.dateTo)
      links = links.filter((l: any) => l.createdAt <= args.dateTo!);
...
export const deleteLink = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (link.userId && link.userId !== identity?.subject)
